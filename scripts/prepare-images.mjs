 ...
 *
 * Both are cleaned up the same way before resizing — a gentle, natural pass:
 ...
 * src/data/projectPhotos.js, src/data/siteImages.js and CREDITS.md.
 */
...
import { PROJECT_PHOTOS, SITE_IMAGES, OG_IMAGE } from './image-sources.mjs'
...
async function emit({ input, name, outDir, options = {}, sizes = WIDTHS }) {
  ...
  return {
    src: `${outDir === projectOut ? '/images/projects' : '/images/site'}/${name}-${fallbackWidth}.jpg`,
    sources: widths.map((width) => ({
      src: `${outDir === projectOut ? '/images/projects' : '/images/site'}/${name}-${width}.webp`,
      width,
    })),
    width: fallbackWidth,
    height,
...
async function buildProjectPhotos() {
  ...
  for (const group of PROJECT_PHOTOS) {
    for (const photo of group.photos) {
      const input = join(projectSource, photo.file)
      if (!existsSync(input)) {
        missing.push(`${photo.name} -> ${photo.file}`)
        continue
      }
...
async function buildSiteImages() {
  ...
  for (const image of SITE_IMAGES) {
    ...
    const entry = await emit({
      ...
      sizes: image.name === 'hero' ? WIDTHS_LARGE : WIDTHS,
    })
...
async function main() {
  ...
  console.log(
    ...
    } site images · CREDITS.md written`
  )
}
